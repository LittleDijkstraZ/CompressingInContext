...
- Debugging compression: set `RKV_DEBUG_COMPRESSION=1` (or `true`/`yes`/`on`) before running to print a line per layer whenever compression runs, e.g. `[R1KV][layer 0] compressed cache 2048 -> 1312 (rotated=False, target_rotation_position=n/a, next_pos=1312)`. That is the only extra debug output aside from standard warnings/logging.
